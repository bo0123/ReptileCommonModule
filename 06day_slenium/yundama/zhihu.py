...
class ZhihuSpider:
    ...
    def login(self):
        ...
        captcha_url = 'https://www.zhihu.com/captcha.gif?r=%d&type=login_start_requests' % (time.time() * 1000)
        ...
        print(post_data)
        resp = self.session.post('https://www.zhihu.com/login_start_requests/email', data=post_data, headers=self.headers)
        print(resp.status_code)
        print(resp.content.decode())
        resposne = self.session.get("https://www.zhihu.com/",headers=self.headers)

        with open("login_start_requests.html","w",encoding="utf-8") as f:
